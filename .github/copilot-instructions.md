...
- Enable plugin “DNA Tools” (Editor → Project Settings → Plugins). It adds docks for DNA validation and monster preview: [addons/dna_tools](monster-farm-gamefiles/monster-farm/addons/dna_tools).
- Quick dev loop: open [scenes/game_world.tscn](monster-farm-gamefiles/monster-farm/scenes/game_world.tscn). `_spawn_test_monsters()` demonstrates assembling via `MonsterAssembler` with sample DNA resources.
- Content authoring is Resource-first: add `.tres` DNA under `data/dna/...`; the assembler picks up fields via the stack and meta data.
...
- Monster pipeline: [docs/monster-assembly-pipeline.md](docs/monster-assembly-pipeline.md)
- Save spec: [docs/save-load-spec.md](docs/save-load-spec.md)

If any area is unclear (e.g., export presets, mod loader API, component locations), tell us what you need and I’ll expand these rules.
